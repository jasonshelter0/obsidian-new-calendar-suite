...
import { getDateFromFile, getDateUID } from "../io/utils";
import { parseNCFilename } from "../utils/nc-dates";
import { getNCPhaseSettings, getNCMonthSettings, getNCSeasonSettings, getNCYearSettings } from "../io/utils";
...
export function getDateUIDFromFile(file: TFile | null): string {
  ...
  if (date) return getDateUID(date, "month");

  // Try NC granularities
  const ncReaders: Record<string, () => { format: string; folder: string }> = {
  ...
  for (const [g, reader] of Object.entries(ncReaders)) {
    try {
      ...
      if (s.format && s.folder) {
        const p = parseNCFilename(file.basename, s.format, g as any);
        if (p) return `nc-${g}-${p.ny.toString().padStart(2,"0")}-${p.nm.toString().padStart(2,"0")}`;
      }
    } catch { /* skip */ }
  }
  return null;
